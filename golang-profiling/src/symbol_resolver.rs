...
use golang_profiling_common::{FuncInfo, GoRuntimeInfo, StackFrame};
...
impl SymbolResolver {
    ...
    fn load_symbols(&mut self) -> Result<()> {
        ...
        info!(
            "Loaded {} function symbols, {} cached symbols",
            self.func_table.len(),
        ...
        if !self.func_table.is_empty() {
            ...
            for (i, func) in self.func_table.iter().take(5).enumerate() {
                debug!(
                    "Function {}: entry=0x{:x}, name_off={}",
                    i, func.entry, func.name_off
                );
    ...
    fn load_symbols_fallback(&mut self, obj: &object::File) -> Result<()> {
        ...
        info!(
            "Loaded {} functions and {} cached symbols",
            self.func_table.len(),
            self.symbol_cache.len()
        );
    ...
    fn parse_pclntab(&mut self, data: &[u8]) -> Result<()> {
        ...
        match self.parse_pclntab_enhanced(data) {
            ...
            Err(e) => {
                warn!(
                    "Enhanced pclntab parser failed: {}, falling back to legacy parser",
                    e
                );
            }
        }
    ...
    fn parse_pclntab_enhanced(&mut self, data: &[u8]) -> Result<()> {
        ...
        info!(
            "Enhanced parser found {} functions (Go version {}, text_start=0x{:x})",
    ...
    fn parse_pclntab_legacy(&mut self, data: &[u8]) -> Result<()> {
        ...
        if magic != 0xfffffffb && magic != 0xfffffff1 && magic != 0xfffffff0 {
            debug!(
                "Unsupported pclntab magic: 0x{:x}, skipping pclntab parsing",
                magic
            );
        ...
        if ptrsize != 4 && ptrsize != 8 {
            debug!(
                "Unsupported pointer size: {}, skipping pclntab parsing",
                ptrsize
            );
            return Ok(()); // Don't fail, just skip pclntab parsing
        }
        ...
        info!(
            "Found {} functions in pclntab (ptrsize={}, magic=0x{:x})",
            nfunctab, ptrsize, magic
        );
        ...
        for i in 0..std::cmp::min(nfunctab, 1000) {
            ...
            let is_likely_string = entry_bytes
                .iter()
                .chain(funcoff_bytes.iter())
                .take(8) // Check first 8 bytes
                .filter(|&&b| b >= 32 && b <= 126) // Printable ASCII
                .count()
                >= 6; // At least 6 out of 8 bytes are printable

            if is_likely_string {
                debug!(
                    "Detected string data at function {}, stopping function table parsing",
                    i
                );
                break;
            }
            ...
            if entry > 0 && entry < 0x10000000 {
                ...
                if i < 10 {
                    debug!(
                        "Function {}: entry=0x{:x} (rel=0x{:x}), funcoff=0x{:x}",
                        i, absolute_entry, entry, funcoff
                    );
                }
            } else {
                ...
                if i < 10 {
                    debug!(
                        "Skipping invalid function {}: entry=0x{:x}, funcoff=0x{:x}",
                        i, entry, funcoff
                    );
                }
            ...
            if data[string_table_start] != 0 && data[string_table_start].is_ascii_alphabetic() {
                ...
                if end > string_table_start {
                    ...
                    if potential_name.len() > 3
                        && (potential_name.contains("main")
                            || potential_name.contains("runtime")
                            || potential_name.contains("internal")
                            || potential_name.contains(".")
                            || potential_name.contains("/"))
                    {
                        debug!(
                            "Found string table at offset 0x{:x}: {}",
                            string_table_start, potential_name
                        );
                        break;
                    }
                }
        ...
        if string_table_start < data.len() {
            ...
            info!(
                "Loaded string table of {} bytes starting at offset {}",
        ...
        for (entry, funcoff) in func_entries {
            ...
            let name_off = if funcoff > 0 {
                ...
                if relative_funcoff + 4 < data.len() as u64 {
                    ...
                    u32::from_le_bytes([
                        data[relative_funcoff as usize],
                        data[relative_funcoff as usize + 1],
                        data[relative_funcoff as usize + 2],
                        data[relative_funcoff as usize + 3],
                    ])
                } else {
                    0
                }
        ...
        info!(
            "Successfully parsed {} functions from pclntab",
            self.func_table.len()
        );
        ...
        for map in &self.process_maps {
            if let procfs::process::MMapPath::Path(pathname) = &map.pathname {
                ...
                if (path_str.contains("test_program") || path_str.contains("golang-profile"))
                    && perms_str.contains('x')
                    && perms_str.contains('r')
                {
                    self.base_address = map.address.0;
                    info!(
                        "Found base address: 0x{:x} for {}",
                        self.base_address, path_str
                    );
                    break;
                }
            }
    ...
    pub fn resolve_pc(&self, pc: u64) -> String {
        ...
        if let Some(ref gopclntab) = self.gopclntab {
            ...
            debug!(
                "Gopclntab lookup: pc=0x{:x}, lookup_pc=0x{:x}, text_start=0x{:x}, base=0x{:x}",
        ...
        if let Some(func_info) = self.find_function_by_pc(pc) {
            debug!(
                "Found function info: entry=0x{:x}, name_off={}",
                func_info.entry, func_info.name_off
            );
    ...
    fn get_complete_function_info(&self, func_info: &FuncInfo, pc: u64) -> Option<String> {
        ...
        match (file_path, line_number) {
            (Some(file), Some(line)) => Some(format!("{} {}:{}", cleaned_name, file, line)),
            (Some(file), None) => Some(format!("{} {}", cleaned_name, file)),
            _ => Some(cleaned_name),
        }
    }
    ...
    fn clean_go_function_name(&self, name: &str) -> String {
        ...
        if cleaned.starts_with("0x") {
            format!(
                "[unknown:{}]",
                &cleaned[2..std::cmp::min(cleaned.len(), 10)]
            )
        } else {
            cleaned
        }
    ...
    fn get_function_name(&self, func_info: &FuncInfo) -> Option<String> {
        ...
        debug!(
            "String table size: {}, name_offset: {}",
            self.string_table.len(),
            name_offset
        );
        if name_offset >= self.string_table.len() {
            debug!(
                "name_offset {} >= string_table.len() {}",
                name_offset,
                self.string_table.len()
            );
            return None;
...
mod tests {
    ...
    use golang_profiling_common::{FuncInfo, StackFrame};
    ...
    fn test_resolve_pc() {
        ...
        assert_eq!(name, "[unknown:0x500000]");
    }
}
