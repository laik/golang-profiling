...
impl Gopclntab {
    pub fn new(data: Vec<u8>) -> Result<Self, String> {
        ...
        let hdr = unsafe { &*(data.as_ptr() as *const PclntabHeader) };
        ...
        match version {
            GO1_2 => {
                let functab_end = hdr_size
                    + gopclntab.num_funcs * gopclntab.func_map_size as usize
                    + hdr.ptr_size as usize;
                let filetab_offset =
                    get_int32(&data, functab_end).ok_or("Failed to read filetab_offset")?;
                ...
                if filetab_offset == 0 || num_source_files == 0 {
                    return Err(format!(
                        ".gopclntab corrupt (filetab 0x{:x}, nfiles {})",
                        filetab_offset, num_source_files
                    ));
                }
            ...
            GO1_16 => {
                ...
                let hdr116 = unsafe { &*(data.as_ptr() as *const PclntabHeader116) };
                ...
                if data_len < funcname_offset
                    || data_len < cu_offset
                    || data_len < filetab_offset
                    || data_len < pctab_offset
                    || data_len < pcln_offset
                {
                    return Err(format!(
                        ".gopclntab is corrupt ({:x}, {:x}, {:x}, {:x}, {:x})",
            ...
            GO1_18 | GO1_20 => {
                ...
                let hdr118 = unsafe { &*(data.as_ptr() as *const PclntabHeader118) };
    ...
    pub fn get_func_map_entry(&self, index: usize) -> (usize, usize) {
        if self.version >= GO1_18 {
            ...
            if offset + 8 <= self.functab.len() {
                let fmap =
                    unsafe { &*(self.functab[offset..].as_ptr() as *const PclntabFuncMap118) };
                return (self.text_start + fmap.pc as usize, fmap.func_off as usize);
            }
        } else {
            let offset = index * self.func_map_size as usize;
            if offset + 16 <= self.functab.len() {
                let fmap = unsafe { &*(self.functab[offset..].as_ptr() as *const PclntabFuncMap) };
    ...
    pub fn get_func(&self, mut func_off: usize) -> Option<(usize, PclntabFunc)> {
        ...
        let func = unsafe { *(self.funcdata[func_off..].as_ptr() as *const PclntabFunc) };
