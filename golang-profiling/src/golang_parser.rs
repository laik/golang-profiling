...
impl GoRuntimeParser {
    ...
    pub fn parse_process(&mut self, pid: u32) -> Result<GoRuntimeInfo> {
        ...
        if let Some((func_tab_base, func_tab_size)) = self.find_func_table(&obj)? {
            ...
            debug!(
                "Function table: base=0x{:x}, size={}",
                func_tab_base, func_tab_size
            );
        }
    ...
    fn parse_goroutine_offsets(
        &self,
        runtime_info: &mut GoRuntimeInfo,
        version: &str,
    ) -> Result<()> {
        // These offsets are version-dependent and architecture-dependent
        // For x86_64, common offsets:

        if version.starts_with("go1.21")
            || version.starts_with("go1.22")
            || version.starts_with("go1.23")
        {
        ...
        debug!(
            "Goroutine offsets: g=0x{:x}, stack_base=0x{:x}, stack_size=0x{:x}",
...
mod tests {
    ...
    fn test_parse_goroutine_offsets() {
        ...
        parser
            .parse_goroutine_offsets(&mut runtime_info, "go1.21.0")
            .unwrap();
        ...
        parser
            .parse_goroutine_offsets(&mut runtime_info, "go1.20.5")
            .unwrap();
        assert_eq!(runtime_info.g_offset, 0x30);
        assert_eq!(runtime_info.stack_base_offset, 0x10);
        assert_eq!(runtime_info.stack_size_offset, 0x18);
        ...
        parser
            .parse_goroutine_offsets(&mut runtime_info, "go1.15.0")
            .unwrap();
        assert_eq!(runtime_info.g_offset, 0x30);
        assert_eq!(runtime_info.stack_base_offset, 0x10);
        assert_eq!(runtime_info.stack_size_offset, 0x18);
    ...
    fn test_update_runtime_addresses() {
        ...
        writeln!(
            temp_file,
            "7f8b40000000-7f8b40001000 r-xp 00000000 08:01 123456 /usr/bin/test"
        )
        .unwrap();
        writeln!(
            temp_file,
            "7f8b50000000-7f8b50001000 rw-p 00000000 00:00 0 [heap]"
        )
        .unwrap();
        writeln!(
            temp_file,
            "7f8b60000000-7f8b60001000 r-xp 00000000 08:01 789012 /go/bin/app"
        )
        .unwrap();
        ...
        parser
            .update_runtime_addresses(&mut runtime_info, temp_file.path().to_str().unwrap())
            .unwrap();
    ...
    fn test_new_parser() {
        ...
        assert_eq!(
            ...
            std::mem::size_of::<GoRuntimeParser>()
        );
    }
