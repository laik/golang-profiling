...
fn main() -> anyhow::Result<()> {
    ...
    aya_build::build_ebpf([ebpf_package]).context("Error building eBPF program")?;
    ...
    if flamegraph_path.exists() {
        let flamegraph_content =
            fs::read_to_string(flamegraph_path).context("Failed to read flamegraph.pl")?;
